...
export class Game {
    ...
    private checkExits() {
        if (!this.player || !this.camera || !this.currentExits.length) return;

        const pRect = this.player.getBounds();
        // Shrink player bounds slightly for more forgiving entry
        const pCenter = { x: pRect.x + pRect.width / 2, y: pRect.y + pRect.height / 2 };

        for (const exit of this.currentExits) {
            // Exit coordinates are relative to room, so add camera
            const exitRect = {
                x: exit.x + this.camera.x,
                y: exit.y + this.camera.y,
                width: exit.width,
                height: exit.height
            };

            if (pCenter.x >= exitRect.x && pCenter.x <= exitRect.x + exitRect.width &&
                pCenter.y >= exitRect.y && pCenter.y <= exitRect.y + exitRect.height) {

                console.log('Entered Exit to:', exit.targetMap, exit.targetRoom);
                this.switchMap(exit.targetMap, exit.targetRoom, exit.targetX, exit.targetY);
                return;
            }
        }
    }
    ...
    private checkSecrets() {
        if (!this.currentSecrets.length) return;

        // Check Bombs
        this.bombs.forEach(bomb => {
            if (bomb.state === BombState.Exploding) {
                const bRect = bomb.getBounds();

                this.currentSecrets.forEach((secret, index) => {
                    if (secret.trigger === 'bomb') {
                        // Secret coordinates are relative to room, so add camera
                        const sRect = {
                            x: secret.x + this.camera!.x,
                            y: secret.y + this.camera!.y,
                            width: 32, // Assume 32x32 for secret area
                            height: 32
                        };

                        if (this.checkCollision(bRect, sRect)) {
                            // Reveal Secret
                            const wall = this.secretWalls[index];
                            if (wall && !wall.isRevealed) {
                                wall.reveal();
                                console.log('Secret Revealed!');

                                // Add Exit
                                this.currentExits.push({
                                    x: secret.x,
                                    y: secret.y,
                                    width: 32,
                                    height: 32,
                                    targetMap: secret.targetMap,
                                    targetRoom: secret.targetRoom,
                                    targetX: 128, // Default center-ish
                                    targetY: 128
                                });
                            }
                        }
    ...
    public switchMap(mapName: string, targetRoom: string, x: number, y: number) {
        if (!this.camera || !this.player) return;
        const newMap = this.maps[mapName];
        if (!newMap) {
        ...
        this.tilemap = new Tilemap(this.currentMap);

        // Parse targetRoom "x,y" to set camera
        const [rx, ry] = targetRoom.split(',').map(Number);
        this.camera.x = rx * this.width;
        this.camera.y = ry * this.height;

        // Set player position relative to new camera
        this.player.x = this.camera.x + x;
        this.player.y = this.camera.y + y;

        this.spawnEnemies();
    }

    // Room Data Removed - Loaded from JSON
    ...
    private startTransition(dir: { x: number, y: number }) {
            case 'map': type = ItemType.Map; break;
            case 'key': type = ItemType.Key; break;
            case 'candle': type = ItemType.Candle; break;
            case 'ladder': type = ItemType.Ladder; break;
            case 'magic_rod': type = ItemType.MagicRod; break;
            case 'blue_ring': type = ItemType.BlueRing; break;
            case 'red_ring': type = ItemType.RedRing; break;
            case 'heart_container': type = ItemType.HeartContainer; break;
    }

        this.items.push(new Item(worldX, worldY, type));
    });
}
    }
